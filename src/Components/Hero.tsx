...
export const Hero: React.FC = () => {
    ...
    return (
        <header className="relative min-h-[100vh] flex items-center justify-center pt-20 pb-12 px-4 md:px-6 max-w-7xl mx-auto overflow-hidden">

            {/* 1. نظام الإضاءة المحيطة المطور */}
            <motion.div style={{ y: bgY }} className="absolute inset-0 z-0 pointer-events-none select-none">
                ...
                {/* توهج سفلي أرجواني */}
                <div className="absolute bottom-[10%] right-[-10%] w-[60%] h-[50%] bg-purple-600/10 blur-[120px] rounded-full" />
                {/* خط ضوئي مائل خلف النص */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-[1px] bg-gradient-to-r from-transparent via-purple-500/20 to-transparent rotate-[-15deg]" />
            </motion.div>
            ...
            >
                ...
                <div className="relative mb-12 group">
                    <motion.div
                        ...
                        className="relative z-10 w-44 h-44 md:w-56 md:h-56 rounded-[3.5rem] overflow-hidden border border-white/10 p-2 bg-slate-900/50 backdrop-blur-3xl shadow-2xl"
                    >
                    ...
                    {/* Badge الحالة (Live Indicator) */}
                    <motion.div
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        transition={{ delay: 0.5, type: 'spring' }}
                        className="absolute -bottom-2 -right-2 md:bottom-2 md:right-2 z-20 flex items-center gap-2 bg-white dark:bg-slate-900 px-4 py-2 rounded-2xl shadow-xl border border-slate-200 dark:border-white/5"
                    >
                ...
                <div className="space-y-8 max-w-4xl">
                    <motion.div
                        ...
                        className="inline-flex items-center gap-3 px-6 py-2 rounded-full border border-white/5 bg-white/[0.02] backdrop-blur-2xl shadow-inner"
                    >
                    ...
                    <h1 className="text-6xl md:text-8xl lg:text-9xl font-black tracking-tighter leading-[0.8] uppercase">
                        ...
                        <span className="block bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 bg-clip-text text-transparent italic font-light pb-2">
                            {t('HERO_NAME_LAST')}
                        </span>
                    </h1>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-6 mt-12 w-full px-4">
                    <motion.a
                        ...
                        whileTap={{ scale: 0.98 }}
                        href="#projects"
                        className="w-full sm:w-auto flex items-center justify-center gap-4 bg-slate-900 dark:bg-white text-white dark:text-black px-10 py-5 rounded-2xl font-black text-xs tracking-widest uppercase transition-all"
                    >
                    ...
                    <div className="flex gap-4">
                        {socialLinks.map((social, idx) => (
                            <motion.a
                                ...
                                className="w-14 h-14 rounded-2xl border border-slate-200 dark:border-white/10 flex items-center justify-center text-slate-500 dark:text-slate-400 transition-all backdrop-blur-md"
                            >
                                {social.icon}
                            </motion.a>
                        ))}
            ...
            {/* 5. مؤشر التمرير العمودي (Scroll Visual) */}
